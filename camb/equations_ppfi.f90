	...
	! Some functions for IDE  
    module IDEtools
        use precision
        use ModelParams
        implicit none
        contains
        function ConformalH(a,grhov_t,grhoc_t) !adotoa 
        use MassiveNu
        real(dl) a,grhov_t,grhoc_t,ConformalH,a2,grho,rhonu 
        integer nu_i 
        a2=a*a 
        grho=grhok+grhob/a+grhov_t+grhoc_t+(grhog+grhornomass)/a2 
        ...
        ConformalH=sqrt(grho/3._dl)
        end function ConformalH
        
        ! Hermite interpolation using ya and its first derivative dya
        ! x locates between xa(klo) and xa(klo+1). 
        function Hermite(xa,ya,dya,x,klo) result(y)  
        real(dl), dimension(:) :: xa,ya,dya
        real(dl) x,y,a,b,h
        integer khi,klo
        khi = klo+1
        h=xa(khi)-xa(klo)
        a=(xa(khi)-x)/h
        b=(x-xa(klo))/h
        y=ya(klo)*(1+2*b)*a**2+ya(khi)*(1+2*a)*b**2+&
            dya(klo)*b*h*a**2-dya(khi)*a*h*b**2
        end function Hermite
        end module IDEtools
        
        ! module for calculating the background evolutions of coupled fluid models 
        ...
        ! A specific coupled fluid model is described by three elements, 
        ! dark energy EoS, energy transfer Q, and covariant    
        integer, parameter :: CPL=1, HDE=2, NADE=3
        integer, parameter :: Hrde=1, Hrc=2, H0rde=3, H0rc=4
        integer, parameter :: uc=1, ude=2	
        type CoupFluidTypes
            integer :: WForm = 1
            integer :: QForm = 1
            integer :: CovQForm = 1
        ...
        !free parameters for coupled fluid models
        type CoupFluidParams
            real(dl) :: w0 = -1_dl ! CPL model parameter
            real(dl) :: w1 = 0._dl ! CPL model parameter
        ...
        ! parameters whose values needed to be obtained by iteration.
        ...
        ! Models with Q proportional to \rho_c may lead to negative value of \rho_{de} at early times,
        ! if we evolve \rho_{de} from today. a_trans denotes the scale factor value when this transition occors.
        real(dl) a_trans  
        
        !for solving the background equations 
        integer, parameter :: nvar = 2  
        integer, parameter :: na = 2000
        integer, parameter :: i_de4 = 1, i_c4 =2
        real(dl), parameter :: amin = 1.d-12
        real(dl) ai(na),ya(na,nvar),dya(na,nvar)
        private nvar,na,i_de4,i_c4,amin,ai,ya,dya,Eqs_CF,ItP,GetCorrect_initial
        contains
        
        ! EoS forms of some DE models
        real(dl) function EoS_CF(a,adotoa,grhov_t,grhoc_t)
        !adotoa = H*a, grhov_t=8\pi G a^2 \rho_de(a), grhoc_t=8 \pi G a^2 \rho_c(a)
        real(dl), intent(in) ::  a,adotoa,grhoc_t,grhov_t
        select case(CFT%WForm)
        case (CPL)
            EoS_CF = CFP%w0+CFP%w1*(1._dl-a) !CPL model        
        case (HDE)
            EoS_CF = -1/3._dl-2*sqrt(abs(grhov_t)/3._dl)/(3*CFP%c*adotoa) !corresponding to HDE model
        case (NADE)
            EoS_CF = -1._dl+2*sqrt(abs(grhov_t)/3._dl)/(3*a*ItP%n*adotoa) !corresponding to NADE model
        case default
            stop 'Invalid w form'
        end select
        end function EoS_CF  ! equation of state of DE
    
        ! 8\pi G a^3 Q
        real(dl) function Coup_CF(a,adotoa,grhov_t,grhoc_t)
        !adotoa = H*a, grhov_t=8\pi G a^2 \rho_de(a), grhoc_t=8 \pi G a^2 \rho_c(a)
        real(dl), intent(in) ::  a,adotoa,grhoc_t,grhov_t
        real(dl) :: wde
        select case(CFT%QForm)
        case (Hrde)
            Coup_CF = CFP%beta*adotoa*grhov_t   !corresponding to Q=\beta H \rho_de model
        case (Hrc)
            Coup_CF = CFP%beta*adotoa*grhoc_t   !corresponding to Q=\beta H \rho_c model
        case (H0rde)
            Coup_CF = CFP%beta*sqrt(grhom/3._dl)*a*grhov_t   !corresponding to Q=\beta H_0 \rho_de model
        case (H0rc)
            Coup_CF = CFP%beta*sqrt(grhom/3._dl)*a*grhoc_t   !corresponding to Q=\beta H_0 \rho_c model
        case default
            !stop 'Invalid coupling form'
            !Coup_CF = CFP%beta*adotoa*grhov_t*grhoc_t/(grhov_t+grhoc_t)  !corresponding to Q=\beta H \rho_de*\rho_c/(\rho_de + \rho_c) model
            wde = EoS_CF(a,adotoa,grhov_t,grhoc_t)
            Coup_CF = -3._dl*wde*CFP%beta*adotoa*grhov_t*grhoc_t/(grhov_t+grhoc_t)  !corresponding to NGCG model. here beta represents alpha in NGCG model 
        end select
        end function Coup_CF
    
        ! gC = 8\pi G a^3 C
        subroutine PerturCoupC_CF(a,adotoa,grhov_t,grhoc_t,gC)
        real(dl), intent(in) ::  a,adotoa,grhoc_t,grhov_t
        real(dl), intent(out) :: gC(3)
        real(dl) :: gQ
        gQ = Coup_CF(a,adotoa,grhov_t,grhoc_t)
        select case(CFT%QForm)
        case (Hrde,H0rde)
            gC(1)=gQ; gC(2)=0._dl; gC(3)=0._dl    
        case (Hrc,H0rc) 
            gC(1)=0._dl; gC(2)=gQ; gC(3)=0._dl   
        case default
            !stop 'Invalid coupling form'
            gC(1)=gQ; gC(2)=gQ; gC(3)=0._dl  !corresponding to NGCG model or Q=\beta H \rho_de*\rho_c/(\rho_de + \rho_c) model
        end select
        end subroutine PerturCoupC_CF
        
        ! gD = 8\pi G a^3 D
        subroutine PerturCoupD_CF(a,adotoa,grhov_t,grhoc_t,gD)
        real(dl), intent(in) ::  a,adotoa,grhoc_t,grhov_t
        ...
        ! if cosmological model is cosmological constant, then perturDE = .false.
        ! if gD(1) = 0, we do not need to evolve vc, then evolveVc = .false.  
        subroutine LogicalPertur_CF(perturDE,evolveVc)
        logical, intent(out) :: perturDE,evolveVc
        real(dl) gD(2)
        perturDE = .true.; evolveVc = .false.
        select case(CFT%WForm)
        case (CPL)
            if( CFP%w0==-1._dl .and. CFP%w1==0._dl .and. CFP%beta==0._dl ) perturDE = .false.
        case (HDE,NADE)
            perturDE = .true. 
        case default
            stop 'Invalid w form'
        end select
        call PerturCoupD_CF(1._dl,1._dl,1._dl,1._dl,gD)
        if(gD(1).ne.0._dl) evolveVc =.true.
        end subroutine LogicalPertur_CF
    
        subroutine InitialCondition_CF(a,y)
        ...
        case (NADE)
            a=amin
            Fa = 1._dl/((grhoc+grhob)/(grhog+grhornomass)*a+1._dl)
            y(i_de4) = 0.75_dl*( adotrad*ItP%n*a*(1._dl+sqrt(Fa)) )**2
            y(i_c4)= grhoc*a**(1._dl+ItP%diff)
        end select
        end subroutine InitialCondition_CF
        
        ! solve background equations from a=1 to a=amin.
        subroutine Solve_CF
        ...
        case (NADE)
            ...
            do j=1,na
                ...
                call dverk(EV,nvar,Eqs_CF,a,y,aend,tol1,ind,c,nvar,w)
                call Eqs_CF(EV,nvar,a,y,dy)
                ai(j) = a; ya(j,:) = y; dya(j,:)=dy
                !if(y(i_de4)<=0._dl .and. a_trans==0._dl) a_trans=a 
            end do
        end select
        end subroutine Solve_CF
    
        subroutine Eqs_CF(EV,n,a,y,dy)
        integer n
        real(dl) EV,a,y(n),dy(n)
        real(dl) a2,adotoa,grhov_t,grhoc_t,gQ,wde
        a2=a*a 
        grhov_t = y(i_de4)/a2; grhoc_t = y(i_c4)/a2
        adotoa = ConformalH(a,grhov_t,grhoc_t)
        wde = EoS_CF(a,adotoa,grhov_t,grhoc_t)
        gQ = Coup_CF(a,adotoa,grhov_t,grhoc_t)
        dy(i_de4) = (1-3*wde)*grhov_t*a + gQ*a/adotoa
        dy(i_c4) = grhoc_t*a - gQ*a/adotoa
        !write(1,'(6E15.5)')  a,wde,y,dy
        end subroutine Eqs_CF
        
        ! after solving background, get grhov_t and grhoc_t at a using hermite intepolation. 
        ...
        integer i
        !open(unit=1,file='test1.txt',form='formatted')
        tol1=1.d-5; delta=1.d-3
        x0(1)=ItP%n; x0(2)=ItP%diff
        !get first derivative matrix of f
        call IteraFcns(x0,f0)
        !write(*,*) 'x0=,f0=' , x0,f0
        x1(1)=x0(1)+delta; x1(2)=x0(2)
        call IteraFcns(x1,f1)
        !write(*,*) 'x1=,f1=' , x1,f1
        D0(:,1)=(f1-f0)/delta
        x1(1)=x0(1); x1(2)=x0(2)+delta
        call IteraFcns(x1,f1)
        !write(*,*) 'x2=,f2=' , x1,f1
        D0(:,2)=(f1-f0)/delta
        !inverse the first derivative matrix of f
        H0(1,1)=D0(2,2); H0(2,1)=-D0(2,1); H0(1,2)=-D0(1,2); H0(2,2)=D0(1,1)
        H0=H0/(D0(1,1)*D0(2,2)-D0(1,2)*D0(2,1))
        !do Broyden iteration
        do i=1,itmax
            ...
            call IteraFcns(x1,f1)
            dx=x1-x0; df=f1-f0
            v1=matmul(H0,df); v2=dx-v1
            t1=dot_product(v2,dx); t2=dot_product(dx,v1)
            H1=(1+t1/t2)*H0
            x0=x1; f0=f1; H0=H1	
            if(i>=itmax) stop 'BroydenIteration not converge'
        end do
        contains
            subroutine IteraFcns(x,f)
            real(dl), dimension(:) :: x,f
            real(dl) EV,c(24),w(nvar,9),y(nvar),tol1,a
            integer ind
            ind = 1; tol1=1.d-8
            ItP%n=x(1); ItP%diff=x(2)
            call InitialCondition_CF(a,y)
            call dverk(EV,nvar,Eqs_CF,a,y,a,tol1,ind,c,nvar,w) !carefully handle the first point 
            !write(*,*) 'y=' , y	
            call dverk(EV,nvar,Eqs_CF,a,y,1._dl,tol1,ind,c,nvar,w) !get y at a=1
            !write(*,*) 'y1=' , y
            f(1) = ( y(i_de4)-grhov )/grhov !give correct grho_{de} at a=1
            f(2) = ( y(i_c4)-grhoc )/grhoc !give correct gphi at a=1
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ! module for calculating the background evolutions of coupled quintessence models 
        module CoupledQuintModels
        use precision
        use ModelParams
        use IDEtools
        implicit none
    
        integer, parameter :: PLpotential=1, EXPpotential=2 
        integer, parameter :: EXPcoupling=1, PLcoupling=2
        type CoupQuintTypes
            integer :: UForm = 1
            integer :: QForm = 1
        end type CoupQuintTypes
        type(CoupQuintTypes) :: CQT
        
        !free parameters for coupled quintessence models
        type CoupQuintParams
            real(dl) :: alpha = 0._dl  !quintessence potential slope parameter.
            real(dl) :: beta = 0._dl  !dimensionless coupling constant 
        end type CoupQuintParams
        type(CoupQuintParams) :: CQP
    
        ! parameters whose values needed to be obtained by iteration.
        type IterationParams
            real(dl) :: gU0 = 0.9_dl !constant in unit of grhov used in quitenssence potential 
            real(dl) :: gphi0 = 0.5_dl  !current value of sqrt(8 pi G) phi
        end type IterationParams
        type(IterationParams), save :: ItP
        ...
        integer, parameter :: nvar = 2  
        integer, parameter :: na = 2000
        integer, parameter :: i_phi = 1, i_pdot2 =2
        real(dl), parameter :: amin = 1.d-12
        real(dl) ai(na),ya(na,nvar),dya(na,nvar)
        private nvar,na,i_phi,i_pdot2,amin,ai,ya,dya,ItP,&
            InitialCondition_CQ,Eqs_CQ,GetCorrect_initial
        contains
    
        ! potential form
        real(dl) function Potential(gphi,deriv)
        ! gphi = \sqrt{8\pi G} \phi
        ! if deriv=0, return 8\pi G U; if deriv=1, return \sqrt{8\pi G} dU/d\phi.
        real(dl), intent(in) :: gphi
        integer, intent(in) :: deriv
        select case (CQT%UForm)
        case (PLpotential)
            Potential = grhov*ItP%gU0*gphi**(-CQP%alpha)
            if(deriv==1) Potential = -CQP%alpha*Potential/gphi
        case (EXPpotential)
            stop 'Not supported currently'
        case default
            stop 'Invalid DE potential'
        end select
        end function Potential
        
        ! 8\pi G a^3 Q 
        real(dl) function Coup_CQ(a,gphi,gphidot)
        ! gphi = \sqrt{8\pi G} \phi, gphidot=\sqrt{8\pi G} \phi'
        real(dl), intent(in) ::  a,gphi,gphidot
        real(dl) :: grhoc_t
        grhoc_t = grhoca2_CQ(a,gphi,gphidot)
        select case (CQT%QForm)
        case (EXPcoupling)
            Coup_CQ = CQP%beta*grhoc_t*gphidot
        case (PLcoupling)
            stop 'Not supported currently'
        case default
            stop 'Invalid coupling form'
        end select
        end function Coup_CQ
    
        ! 8\pi G a^2 \rho_c(a)
        real(dl) function grhoca2_CQ(a,gphi,gphidot)
        ! gphi = \sqrt{8\pi G} \phi, gphidot=\sqrt{8\pi G} \phi'
        real(dl), intent(in) ::  a,gphi,gphidot
        select case (CQT%QForm)
        case (EXPcoupling)
            grhoca2_CQ = grhoc/a*exp(-CQP%beta*(gphi-ItP%gphi0))
        case (PLcoupling)
            stop 'Not supported currently'
        case default
            stop 'Invalid coupling form'
        end select
        end function grhoca2_CQ
    
        ! gC = 8\pi G a^3 C    
        subroutine PerturCoupC_CQ(a,gphi,gphidot,gC)
        ! gphi = \sqrt{8\pi G} \phi, gphidot=\sqrt{8\pi G} \phi'
        real(dl), intent(in) ::  a,gphi,gphidot
        real(dl), intent(out) :: gC(3)
        real(dl) :: gQ,gU,dgU,wde
        gQ = Coup_CQ(a,gphi,gphidot)
        gU = Potential(gphi,0)
        dgU = Potential(gphi,1)
        wde=(0.5*(gphidot/a)**2-gU)/(0.5*(gphidot/a)**2+gU)
        select case(CQT%QForm)
        case (EXPcoupling)
            gC(1)=gQ/(1+wde); gC(2)=gQ; gC(3)=-gQ*a**2*dgU/gphidot
        case (PLcoupling)
            stop 'Not supported currently'
        case default
            stop 'Invalid coupling form'
        end select
        end subroutine PerturCoupC_CQ
    
        ! gD = 8\pi G a^3 D
        subroutine PerturCoupD_CQ(a,gphi,gphidot,gD)
        ! gphi = \sqrt{8\pi G} \phi, gphidot=\sqrt{8\pi G} \phi'
        real(dl), intent(in) ::  a,gphi,gphidot
        real(dl), intent(out) :: gD(2)
        real(dl) :: gQ
        gQ = Coup_CQ(a,gphi,gphidot)
        gD(1)=gQ; gD(2)=0._dl
        end subroutine PerturCoupD_CQ
    
        ! if cosmological model is cosmological constant, then perturDE = .false.
        ! if gD(1) = 0, we do not need to evolve vc, then evolveVc = .false. 
        subroutine LogicalPertur_CQ(perturDE,evolveVc)
        logical, intent(out) :: perturDE,evolveVc
        real(dl) gD(2)
        perturDE = .true.; evolveVc = .false.
        if( CQP%alpha==0._dl .and. CQP%beta==0._dl ) perturDE = .false.
        call PerturCoupD_CQ(1._dl,1._dl,1._dl,gD)
        if(gD(1).ne.0._dl) evolveVc =.true.
        end subroutine LogicalPertur_CQ
    
        ! initial values of \sqrt{8\pi G} \phi and \sqrt{8\pi G} a^2 \phi' at a=amin
        subroutine InitialCondition_CQ(a,y)
        real(dl), intent(out) :: a,y(nvar)
        real(dl) const
        select case(CQT%UForm)
        case (PLpotential)
            a=amin
            const=CQP%alpha*(2+CQP%alpha)**2*grhov*ItP%gU0
            const=const/( 4*adotrad**2*(6+CQP%alpha) )
            const=const**(1/(2._dl+CQP%alpha))
            y(i_phi) = const*a**(4/(2._dl+CQP%alpha))
            y(i_pdot2)=y(i_phi)*4*adotrad/(2._dl+CQP%alpha)*a
        case (EXPpotential)
            stop 'Not supported currently'
        case default
            stop 'Invalid potential form'
        end select
        end subroutine InitialCondition_CQ  
    
        ! solve background equations from a=amin to a=1.
        subroutine Solve_CQ 
        real(dl) EV,c(24),w(nvar,9),y(nvar),dy(nvar),tol1,a,aend
        integer ind,j
        ind = 1; tol1=1.d-8
        call GetCorrect_initial
        call InitialCondition_CQ(a,y)
        do j=1,na
            aend = amin**((na-j)/(na-1._dl))
            call dverk(EV,nvar,Eqs_CQ,a,y,aend,tol1,ind,c,nvar,w)
            call Eqs_CQ(EV,nvar,a,y,dy)
            ai(j) = a; ya(j,:) = y; dya(j,:)=dy
        end do
        end subroutine Solve_CQ
        
        subroutine Eqs_CQ(EV,n,a,y,dy)
        integer n 
        real(dl) EV,a,y(n),dy(n)
        real(dl) a2,gphi,gphidot,gU,dgU,adotoa,grhov_t,grhoc_t,gQ
        a2=a*a 
        gphi = y(i_phi); gphidot = y(i_pdot2)/a2
        gU = Potential(gphi,0); dgU = Potential(gphi,1)
        grhov_t = 0.5d0*gphidot**2+a2*gU; grhoc_t = grhoca2_CQ(a,gphi,gphidot)
        adotoa = ConformalH(a,grhov_t,grhoc_t)
        gQ = Coup_CQ(a,gphi,gphidot)
        dy(i_phi) = gphidot/adotoa/a
        dy(i_pdot2) = (-a2*dgU+gQ/gphidot)*a/adotoa
        end subroutine Eqs_CQ 
    
        ! after solving background, get gphi and gphidot at a using hermite intepolation. 
        subroutine Get_gphi_gphidot(a,gphi,gphidot)
        real(dl), intent(in) :: a  
        real(dl), intent(out) :: gphi,gphidot
        integer klo
        klo = int( na-(na-1)*log(a)/log(amin) ) 
        klo = max(min(klo,na-1),1)
        gphi = Hermite(ai,ya(:,i_phi),dya(:,i_phi),a,klo)
        gphidot = Hermite(ai,ya(:,i_pdot2),dya(:,i_pdot2),a,klo)/a**2
        end subroutine Get_gphi_gphidot
        
        ! get the correct values of gU0 and gphi0 using Broyden iteration method.
        subroutine GetCorrect_initial
        integer, parameter :: n=2, itmax=100
        real(dl), dimension(n) :: x0,x1,f0,f1,dx,df,v1,v2
        real(dl),dimension(n,n) :: D0,H0,H1
        ...
        integer i
        tol1=1.d-5; delta=1.d-3
        x0(1)=ItP%gU0; x0(2)=ItP%gphi0
        !get first derivative matrix of f
        call IteraFcns(x0,f0)
        x1(1)=x0(1)*(1+delta); x1(2)=x0(2)
        call IteraFcns(x1,f1)
        D0(:,1)=(f1-f0)/(x0(1)*delta)
        x1(1)=x0(1); x1(2)=x0(2)*(1+delta)
        call IteraFcns(x1,f1)
        D0(:,2)=(f1-f0)/(x0(2)*delta)
        !inverse the first derivative matrix of f
        H0(1,1)=D0(2,2); H0(2,1)=-D0(2,1); H0(1,2)=-D0(1,2); H0(2,2)=D0(1,1)
        H0=H0/(D0(1,1)*D0(2,2)-D0(1,2)*D0(2,1))
        !do Broyden iteration
        ...
        contains
            ...
            integer ind
            ind = 1; tol1=1.d-8
            ItP%gU0=x(1); ItP%gphi0=x(2)
            call InitialCondition_CQ(a,y)
            call dverk(EV,nvar,Eqs_CQ,a,y,a,tol1,ind,c,nvar,w) !carefully handle the first point        
            call dverk(EV,nvar,Eqs_CQ,a,y,1._dl,tol1,ind,c,nvar,w) !get y at a=1
            f(1) = ( 0.5*y(i_pdot2)**2+Potential(y(i_phi),0)-grhov )/grhov !give correct grho_{de} at a=1
            f(2) = ( ItP%gphi0-y(i_phi) )/y(i_phi) !give correct gphi at a=1
            end subroutine IteraFcns
        end subroutine GetCorrect_initial
    
        end module CoupledQuintModels
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        logical :: evolve_vc = .false.
        integer, parameter :: CoupFluid=1, CoupQuint=2
        integer :: IDE_Class = -1 !any value less than 1 corresponding to default non-coupled CPL model 
        character(LEN=1024) :: test_output_root = '' !output the IDE quantities, just for test
        contains
    
        subroutine IDEout(a,grhov_t,grhoc_t,wde,gQ,gC,gD,ca2) 
        !defined for the background and perturbation calculations.
        !adotoa = H*a, grhov_t=8\pi G a^2 \rho_de(a), grhoc_t=8 \pi G a^2 \rho_c(a)
        real(dl), intent(in) :: a
        real(dl), intent(out) :: grhov_t,grhoc_t
        real(dl), intent(out), optional :: wde,ca2,gQ,gC(3),gD(2)
        real(dl) a2,adotoa,gphi,gphidot,gU,dgU
        a2=a*a
        if(.not.w_perturb) then
            grhov_t=grhov*a2
            grhoc_t=grhoc/a
            ...
            if (present(ca2)) ca2 = wde 
            return
        end if
        select case(IDE_Class)
        case (CoupFluid)
            call Get_grhodea2_grhoca2(a,grhov_t,grhoc_t)
            adotoa = ConformalH(a,grhov_t,grhoc_t)
            if (present(wde)) wde = EoS_CF(a,adotoa,grhov_t,grhoc_t)
            if (present(gQ)) gQ = Coup_CF(a,adotoa,grhov_t,grhoc_t)
            if (present(gC)) call PerturCoupC_CF(a,adotoa,grhov_t,grhoc_t,gC)
            if (present(gD)) call PerturCoupD_CF(a,adotoa,grhov_t,grhoc_t,gD)
            if (present(ca2)) ca2 = wde !This is just right for constant w, but enough.
        case (CoupQuint) 
            call Get_gphi_gphidot(a,gphi,gphidot)
            gU = Potential(gphi,0)
            grhov_t = 0.5d0*gphidot**2+a2*gU
            grhoc_t = grhoca2_CQ(a,gphi,gphidot)
            if (present(wde)) wde = -1._dl + gphidot**2/grhov_t
            if (present(gQ)) gQ = Coup_CQ(a,gphi,gphidot)
            if (present(gC)) call PerturCoupC_CQ(a,gphi,gphidot,gC)
            if (present(gD)) call PerturCoupD_CQ(a,gphi,gphidot,gD)
            if (present(ca2)) then
                dgU = Potential(gphi,1)
                adotoa=ConformalH(a,grhov_t,grhoc_t)
                ca2=1-2*a2*gphidot*dgU/(gQ-3*adotoa*gphidot**2)
            end if  
        case default
            grhov_t=grhov*a**(-1._dl-3.*w_lam-3.*wa_ppf)*exp(-3.*wa_ppf*(1._dl-a))
            grhoc_t=grhoc/a
            if (present(wde)) wde = w_lam+wa_ppf*(1._dl-a)
            if (present(gQ)) gQ = 0._dl
            if (present(gC)) gC = 0._dl
            if (present(gD)) gD = 0._dl
            if (present(ca2)) ca2 = wde !This is just right for constant w, but enough.
        ...
        subroutine SetIDE
        real(dl) a,z,grhov_t,grhoc_t,adotoa,wde,gQ, gphi,gphidot,gU,dgU
        integer j
        logical :: output_bk = .true.
        select case(IDE_Class)
        case (CoupFluid)
            call LogicalPertur_CF(w_perturb,evolve_vc)
            if(w_perturb) call Solve_CF
        case (CoupQuint)
            call LogicalPertur_CQ(w_perturb,evolve_vc)
            if(w_perturb) call Solve_CQ
        case default
            if(w_lam==-1._dl.and.wa_ppf==0._dl) w_perturb = .false.
            evolve_vc =.false.
        end select
        
        ! jiust for test
        if (test_output_root/='') then
            !call CreateTxtFile('output_bk.txt',1)
            open(unit=1,file=trim(test_output_root)//'.quantity',form='formatted')
            do j=1,2000
                a = 1.0d-4**((2000-j)/(2000-1._dl))
                z = 1.0/a-1
                call IDEout(a,grhov_t,grhoc_t,wde,gQ)
                call Get_gphi_gphidot(a,gphi,gphidot)
                gU = Potential(gphi,0)
                dgU = Potential(gphi,1)
                adotoa = ConformalH(a,grhov_t,grhoc_t)
                write (1,'(11E15.5)') a, z, grhov_t, grhoc_t, adotoa, wde, gQ ,gphi,gphidot,gU,dgU
            end do	
            close(1)
            !stop 'output_bk finished'	
        end if 
        end subroutine SetIDE
    ...
    subroutine init_background
        ...
        !is_cosmological_constant = .not. use_tabulated_w .and. w_lam==-1_dl .and. wa_ppf==0._dl !not used in IDE
        call SetIDE !solving IDE background
